...
abstract class Role implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Blog\\Map\\RoleTableMap';
    ...
    /**
     * The value for the canmoderate field.
     *
     * Note: this column has a database default value of: false
     * @var        boolean
     */
    ...
    /**
     * The value for the canwrite field.
     *
     * Note: this column has a database default value of: false
     * @var        boolean
     */
    ...
    /**
     * The value for the canadministrate field.
     *
     * Note: this column has a database default value of: false
     * @var        boolean
     */
    protected $canadministrate;
    ...
    /**
     * Initializes internal state of Blog\Base\Role object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [canmoderate] column value.
     *
     * @return boolean
     */
     ...
     * @return boolean
     */
    public function isCanmoderate()
    {
        return $this->getCanmoderate();
    }

    /**
     * Get the [canwrite] column value.
     *
     * @return boolean
     */
     ...
     * @return boolean
     */
    public function isCanwrite()
    {
        return $this->getCanwrite();
    }

    /**
     * Get the [canadministrate] column value.
     *
     * @return boolean
     */
    public function getCanadministrate()
    {
     ...
     * @return boolean
     */
    public function isCanadministrate()
    {
        return $this->getCanadministrate();
    }
    ...
    public function setDisplay($v)
    {
        ...
        if ($this->display !== $v) {
            $this->display = $v;
            $this->modifiedColumns[RoleTableMap::COL_DISPLAY] = true;
        }
    ...
    /**
     * Sets the value of the [canmoderate] column.
     ...
     * @return $this The current object (for fluent API support)
     */
    public function setCanmoderate($v)
    {
        if ($v !== null) {
            if (is_string($v)) {
        ...
        if ($this->canmoderate !== $v) {
            $this->canmoderate = $v;
            $this->modifiedColumns[RoleTableMap::COL_CANMODERATE] = true;
        }
    ...
    /**
     * Sets the value of the [canwrite] column.
     * Non-boolean arguments are converted using the following rules:
     *   * 1, '1', 'true',  'on',  and 'yes' are converted to boolean true
     *   * 0, '0', 'false', 'off', and 'no'  are converted to boolean false
     * Check on string values is case insensitive (so 'FaLsE' is seen as 'false').
    ...
    public function setCanwrite($v)
    {
        ...
        if ($this->canwrite !== $v) {
            $this->canwrite = $v;
            $this->modifiedColumns[RoleTableMap::COL_CANWRITE] = true;
        }
    ...
    /**
     * Sets the value of the [canadministrate] column.
     * Non-boolean arguments are converted using the following rules:
     *   * 1, '1', 'true',  'on',  and 'yes' are converted to boolean true
     *   * 0, '0', 'false', 'off', and 'no'  are converted to boolean false
     * Check on string values is case insensitive (so 'FaLsE' is seen as 'false').
     ...
     * @return $this The current object (for fluent API support)
     */
    public function setCanadministrate($v)
    {
        if ($v !== null) {
            if (is_string($v)) {
                $v = in_array(strtolower($v), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        ...
        if ($this->canadministrate !== $v) {
            $this->canadministrate = $v;
            $this->modifiedColumns[RoleTableMap::COL_CANADMINISTRATE] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : RoleTableMap::translateFieldName('Display', TableMap::TYPE_PHPNAME, $indexType)];
            $this->display = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RoleTableMap::translateFieldName('Canmoderate', TableMap::TYPE_PHPNAME, $indexType)];
            $this->canmoderate = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : RoleTableMap::translateFieldName('Canwrite', TableMap::TYPE_PHPNAME, $indexType)];
            $this->canwrite = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : RoleTableMap::translateFieldName('Canadministrate', TableMap::TYPE_PHPNAME, $indexType)];
            $this->canadministrate = (null !== $col) ? (boolean) $col : null;
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Blog\\Role'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(RoleTableMap::COL_DISPLAY)) {
            $modifiedColumns[':p' . $index++]  = 'display';
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANMODERATE)) {
            $modifiedColumns[':p' . $index++]  = 'canModerate';
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANWRITE)) {
            $modifiedColumns[':p' . $index++]  = 'canWrite';
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANADMINISTRATE)) {
            $modifiedColumns[':p' . $index++]  = 'canAdministrate';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'display':
                        ...
                        break;
                    case 'canModerate':
                        $stmt->bindValue($identifier, (int) $this->canmoderate, PDO::PARAM_INT);

                        break;
                    case 'canWrite':
                        $stmt->bindValue($identifier, (int) $this->canwrite, PDO::PARAM_INT);

                        break;
                    case 'canAdministrate':
                        $stmt->bindValue($identifier, (int) $this->canadministrate, PDO::PARAM_INT);
    ...
    public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = [], bool $includeForeignObjects = false): array
    {
        ...
        $result = [
            ...
            $keys[2] => $this->getCanmoderate(),
            $keys[3] => $this->getCanwrite(),
            $keys[4] => $this->getCanadministrate(),
        ];
    ...
    public function setByPosition(int $pos, $value)
    {
        switch ($pos) {
            ...
            case 1:
                ...
                break;
            case 2:
                $this->setCanmoderate($value);
                break;
            case 3:
                $this->setCanwrite($value);
                break;
            case 4:
                $this->setCanadministrate($value);
                break;
        } // switch()
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setCanmoderate($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setCanwrite($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setCanadministrate($arr[$keys[4]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(RoleTableMap::COL_DISPLAY)) {
            $criteria->add(RoleTableMap::COL_DISPLAY, $this->display);
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANMODERATE)) {
            $criteria->add(RoleTableMap::COL_CANMODERATE, $this->canmoderate);
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANWRITE)) {
            $criteria->add(RoleTableMap::COL_CANWRITE, $this->canwrite);
        }
        if ($this->isColumnModified(RoleTableMap::COL_CANADMINISTRATE)) {
            $criteria->add(RoleTableMap::COL_CANADMINISTRATE, $this->canadministrate);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Blog\Role (or compatible) type.
    ...
    public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true): void
    {
        ...
        $copyObj->setCanmoderate($this->getCanmoderate());
        $copyObj->setCanwrite($this->getCanwrite());
        $copyObj->setCanadministrate($this->getCanadministrate());
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Blog\Role Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function initUsers(bool $overrideExisting = true): void
    {
        ...
        $this->collUsers->setModel('\Blog\User');
    }
        ...
        if (null === $this->collUsers || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collUsers) {
                    ...
                    $collUsers->setModel('\Blog\User');

                    return $collUsers;
                }
            } else {
                $collUsers = ChildUserQuery::create(null, $criteria)
                    ->filterByRolesObj($this)
                    ->find($con);
    ...
    public function clear()
    {
        $this->id = null;
        $this->display = null;
        $this->canmoderate = null;
        $this->canwrite = null;
        $this->canadministrate = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
        $this->applyDefaultValues();
        $this->resetModified();
