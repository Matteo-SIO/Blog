...
abstract class Comment implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Blog\\Map\\CommentTableMap';
     ...
     * @var        ChildUser
     */
    protected $aPublisherObj;

    /**
     * @var        ChildArticle
     */
    protected $aArticleObj;
    ...
    /**
     * Initializes internal state of Blog\Base\Comment object.
     * @see applyDefaults()
     */
    ...
    public function setPublisherId($v)
    {
        ...
        if ($this->publisher_id !== $v) {
            $this->publisher_id = $v;
            $this->modifiedColumns[CommentTableMap::COL_PUBLISHER_ID] = true;
        }

        if ($this->aPublisherObj !== null && $this->aPublisherObj->getId() !== $v) {
            $this->aPublisherObj = null;
        }
    ...
    public function setArticleId($v)
    {
        ...
        if ($this->aArticleObj !== null && $this->aArticleObj->getId() !== $v) {
            $this->aArticleObj = null;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CommentTableMap::translateFieldName('Createdat', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->createdat = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CommentTableMap::translateFieldName('PublisherId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Blog\\Comment'), 0, $e);
        }
    }
    ...
    {
        if ($this->aPublisherObj !== null && $this->publisher_id !== $this->aPublisherObj->getId()) {
            $this->aPublisherObj = null;
        }
        if ($this->aArticleObj !== null && $this->article_id !== $this->aArticleObj->getId()) {
            $this->aArticleObj = null;
        }
    }
    ...
    protected function doSave(ConnectionInterface $con): int
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aPublisherObj !== null) {
                if ($this->aPublisherObj->isModified() || $this->aPublisherObj->isNew()) {
                    $affectedRows += $this->aPublisherObj->save($con);
                }
                $this->setPublisherObj($this->aPublisherObj);
            }

            if ($this->aArticleObj !== null) {
                if ($this->aArticleObj->isModified() || $this->aArticleObj->isNew()) {
                    $affectedRows += $this->aArticleObj->save($con);
                }
                $this->setArticleObj($this->aArticleObj);
            }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_PUBLISHER_ID)) {
    ...
    public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = [], bool $includeForeignObjects = false): array
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aPublisherObj) {
                ...
                $result[$key] = $this->aPublisherObj->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
            if (null !== $this->aArticleObj) {
                ...
                $result[$key] = $this->aArticleObj->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_PUBLISHER_ID)) {
            $criteria->add(CommentTableMap::COL_PUBLISHER_ID, $this->publisher_id);
        }
        if ($this->isColumnModified(CommentTableMap::COL_ARTICLE_ID)) {
    ...
    public function hashCode()
    {
        $validPk = null !== $this->getId() &&
            null !== $this->getPublisherId() &&
            null !== $this->getArticleId();

        $validPrimaryKeyFKs = 2;
        $primaryKeyFKs = [];

        //relation comments_fk_9f4a61 to table users
        if ($this->aPublisherObj && $hash = spl_object_hash($this->aPublisherObj)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }

        //relation comments_fk_0ff210 to table articles
        if ($this->aArticleObj && $hash = spl_object_hash($this->aArticleObj)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }
    ...
    public function getPrimaryKey()
    {
        $pks = [];
        $pks[0] = $this->getId();
        $pks[1] = $this->getPublisherId();
        $pks[2] = $this->getArticleId();
    ...
    {
        $this->setId($keys[0]);
        $this->setPublisherId($keys[1]);
        $this->setArticleId($keys[2]);
    }
    ...
    {
        return (null === $this->getId()) && (null === $this->getPublisherId()) && (null === $this->getArticleId());
    }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Blog\Comment (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Blog\Comment Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function setPublisherObj(ChildUser $v = null)
    {
        if ($v === null) {
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getPublisherObj(?ConnectionInterface $con = null)
    {
        if ($this->aPublisherObj === null && ($this->publisher_id != 0)) {
            $this->aPublisherObj = ChildUserQuery::create()->findPk($this->publisher_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPublisherObj->addComments($this);
             */
        }

        return $this->aPublisherObj;
    }
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function setArticleObj(ChildArticle $v = null)
    {
        if ($v === null) {
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getArticleObj(?ConnectionInterface $con = null)
    {
        if ($this->aArticleObj === null && ($this->article_id != 0)) {
            $this->aArticleObj = ChildArticleQuery::create()->findPk($this->article_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aArticleObj->addComments($this);
             */
        }

        return $this->aArticleObj;
    }
    ...
    {
        if (null !== $this->aPublisherObj) {
            $this->aPublisherObj->removeComment($this);
        }
        if (null !== $this->aArticleObj) {
            $this->aArticleObj->removeComment($this);
        }
        $this->id = null;
        $this->createdat = null;
